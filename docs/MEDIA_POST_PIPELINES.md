...
视频模型自带对白/音效适合快速创意验证，但可能漏词、抢拍、压缩时长或口型漂移；正式短剧必须先生成外部主音轨，并将 1–3 个 HTTPS `reference_audio_urls` 传给 Agnes 做口型参考。生成后用同一外部主音轨替换 Provider 音轨；没有外部主音轨、公网参考 URL 或主轨重混收据时，保持 `BLOCKED`。内置声音只有显式标记为 `RAPID_SAMPLE` 时才允许存在，且不得进入交付。
