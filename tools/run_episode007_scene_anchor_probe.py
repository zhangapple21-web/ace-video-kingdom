...
for sid,anchor,prompt in shots:
 cmd=[sys.executable,str(root/"tools/run_short_clip.py"),"--shot-id",sid,"--prompt",prompt,"--manifest",str(manifest),"--output",str(out/f"{sid}.mp4"),"--model","agnes-video-v2.0","--image",str(root/"media_staging/episode_007_virtual_data/anchors/scene_action"/anchor),"--width","704","--height","1280","--num-frames","241","--frame-rate","24","--timeout","900"]
