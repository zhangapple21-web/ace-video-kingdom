...
for sid, prompt in repairs.items():
    cmd = [sys.executable, str(root/"tools/run_short_clip.py"), "--shot-id", sid,
           "--prompt", prompt, "--manifest", str(manifest), "--output", str(out/f"{sid}.mp4"),
           "--model", "agnes-video-v2.0", "--image", str(ref), "--width", "704", "--height", "1280",
           "--num-frames", "121", "--frame-rate", "24", "--timeout", "900"]
